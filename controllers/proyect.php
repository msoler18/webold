...
ini_set('display_errors', 0);
	...
	public static $publicado = array('0' =>'Pendiente Revisión', '1' => 'publicado');
		...
		public static function order_proyects()
		{

			foreach($_POST['array'] as $k => $v) :

				dump(ProyectModel::update(array('order_' => $v), ' WHERE id = '.$k.' '));
		...
		public static function insert_proyect()
		{
			...
			if (isset($_FILES["google_maps"]["name"]) && ($_FILES["google_maps"]["name"] <> '') ) {
				...
				if(move_uploaded_file($teaser_googleNameTemp,$teaser_googleNameNew)) {
					$_POST['google_maps']['logo'] = $teaser_google ;
					 ...
					 echo'Error al copiar la imagen (Ln: 472)';
				}
			}else {
				// $teaser_img  = '';
				 echo 'Error al cargar el nombre de google maps (Ln: 476)';
			}	
			...
			//RECIBE LA DIRECCION Y LA CONVIERTE EN UN ARRAY PARA GUARDARLA
			$dir = __upload_file(PATH.'assets/upload/img/', $_FILES['dir'], PATH.'assets/upload/img/tmp/');
			if ($dir != null): $_POST['dir']['imagen'] = $dir; endif;
		 	$_POST['dir']['content'] = str_replace("\r\n", "", $_POST['dir']['content']);
     	    $_POST['dir']['content']= trim($_POST['dir']['content']);
   		    $_POST['dir'] = json_encode($_POST['dir'], JSON_UNESCAPED_UNICODE);
			
			//MODIFICA LAS CARACTERISTICAS PARA PODERLAS GUARDAR
			...
			$guardar = ProyectModel::save($_POST);
			if($guardar) :
				$url = URL."admin/proyects";
				url_redirect($url);
			else:
				echo "Error al guardar el proyecto (Ln: 590)";
			endif;
		...
		public function update_by_proyect()
		{
			...
			if (isset($_FILES["pdf_mail"]["name"]) && ($_FILES["pdf_mail"]["name"] <> '') ) {
				$teaser_pdf_mail 			= $_FILES["pdf_mail"]["name"];
				$teaser_pdf_mail_NameTemp 	= $_FILES["pdf_mail"]["tmp_name"];
				$teaser_pdf_mail_NameNew 	= $Pathpdf . basename($teaser_pdf_mail);

				// Si se pudo copiar la nueva imágen en el destino
				if(move_uploaded_file($teaser_pdf_mail_NameTemp,$teaser_pdf_mail_NameNew)) {
					
					$_POST['pdf_mail'] = $teaser_pdf_mail ;
		...
		public static function add_proyect_form()
		{
			...
			$proyect['ciudades'] = static::$ciudades;	
		 	...
		 	$proyect['publicados']  = static::$publicado;	
